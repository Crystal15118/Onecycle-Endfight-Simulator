...
public class main {
    ...
    private static JPanel createInputPanel() {
        ...
        filePathField = new JTextField(System.getProperty("user.home") + File.separator + "New folder" + File.separator + "SSGOneCycleRawPerchData.csv");
        gbc.gridx = 1;
        gbc.gridy = 0;
        gbc.gridwidth = 2;
                    ...
                    for (int seedNum = 0; seedNum < seeds.size() && running; ++seedNum) {
                        try {
                            ...
                            for (int count = 0; count < numSims && running; ++count) {
                                ...
                                if (tick + 171 < bedData[seedNum].length) {
                                    ++bedData[seedNum][tick + 171];
                                } else {
                                    publish("Warning: Tick index " + (tick + 171) + " out of bounds.");
                                }
